...
%%
[buttonLocsSampsCellInd,buttonLocs,data,tEpoch] = combine_pooled_data(sid,epochedCortEco_cell,t_epoch);
...
if icaProcess
    ...
    if (condIntAns == 2 || condIntAns == 3|| condIntAns == 4 || condIntAns == 5)
        ...
        %             ica_artifact_remove_train(tEpoch,epochedCortEco,stimChans,fs_data,scale_factor,numComponentsSearch,plotIt,chanInt,meanSub);
        %
        ...
        % %
        %         [processedSig,~,~,~,t] = ...
        %             ica_artifact_remove_train(tEpoch,epochedCortEco,stimChans,fs_data,scale_factor,numComponentsSearch,plotIt,chanInt,meanSub,orderPoly);
        %
        [processedSig,~,~,~,t] = ...
            ica_train(tEpoch,dataInt,stimChans,fs_data,scale_factor,numComponentsSearch,plotIt,chanInt,meanSub,orderPoly);
...
for ind = chanIntList
    tEpoch = (-sampsPreStim:sampsPostStim-1)/fs_data;
...
t_morlet = linspace(-preStim,postStim,length(t_morlet))/1e3;
...
tEpoch = (-samps_pre_stim:samps_post_stim-1)/fs_data;
...
t_sub = tEpoch((tEpoch>t1 & tEpoch<t2));
plv_sub = plv((tEpoch>t1 & tEpoch<t2),:,:);
...
tEpoch = [artifact_end:artifact_end+size(epochedCortEco,1)-1]/fs_data;
tEpochPre = [-presamps:0-1]/fs_data;
...
subplot(2,1,1)
plot(1e3*tEpoch,exampChanPost);
xlabel('time (ms)')
ylabel('Voltage (V)')
title(['Post stim Raw data for Channel ', num2str(chanInt)])

subplot(2,1,2)
plot(1e3*tEpochPre,exampChanPre);
xlabel('time (ms)')
ylabel('Voltage (V)')
...
if strcmp(trimEnds,'y')
    ...
    tEpochT = tEpoch(tEpoch>t_min & tEpoch<t_max);
    tEpochPreT = tEpochPre(tEpochPre>t_minPre & tEpochPre<t_maxPre);
    trialHGT = trialHG(tEpoch>t_min & tEpoch<t_max,:,:);
    trialBetaT = trialBeta(tEpoch>t_min & tEpoch<t_max,:,:);
    restHGT = restHG(tEpochPre>t_minPre & tEpochPre<t_maxPre,:,:);
    restBetaT = restBeta(tEpochPre>t_minPre & tEpochPre<t_maxPre,:,:);
    
    clear tEpoch tEpochPre trialHG trialBeta restHG restBeta
    
    tEpoch = tEpochT;
    tEpochPre = tEpochPreT;
    trialHG = trialHGT;
...
figure
imagesc(1e3*tEpoch,trial,squeeze(trialHGsort(:,chanInt,:))')
axis xy
xlabel('Time (ms)')
ylabel('Trial')
...
figure
imagesc(1e3*tEpoch,trial,squeeze(trialBetasort(:,chanInt,:))')
axis xy
xlabel('Time (ms)')
ylabel('Trial')
...
[C_post, ~, C_totPost, ~] = time_frequency_wavelet(squeeze(epochedCortEco(tEpoch>t_min & tEpoch<t_max,chanInt,:)), fw, fs_data, 1, 1, 'CPUtest');
[C_pre, ~, C_totPre, ~] = time_frequency_wavelet(squeeze(epochedPreStim(tEpochPre>t_minPre &tEpochPre<t_maxPre,chanInt,:)), fw, fs_data, 1, 1, 'CPUtest');
C_norm = normalize_data(C_post',C_pre');

figure
imagesc(1e3*tEpoch,fw,C_norm);
...
figure
imagesc(1e3*tEpochPre,fw,C_pre');
%set_colormap_threshold(gcf, [-1 1], [-7 7], [1 1 1]);
colorbar
axis xy
xlabel('time (ms)');
...
figure
tEpoch = [0:size(epochedButton,1)-1]/fs_stim;
