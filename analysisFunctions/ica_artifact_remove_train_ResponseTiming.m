function [subtracted_sig_matrixS_I, subtracted_sig_cellS_I,recon_artifact_matrix,recon_artifact,t] =  ica_artifact_remove_train_ResponseTiming(tTotal,data,stimChans,fs_data,scale_factor,numComponentsSearch,plotIt,channelInt,meanSub,orderPoly,pre,post)
...
if plotIt
    ...
    for j = 1:size(dataIntTime,3)
        figure
        numInt = min(size(i_icasigS{j},1),10);
...
for i = 1:numTrials
    ...
    for j = 1:numComponentsSearch
        % have to tune this
        [pk_temp_pos,locs_temp_pos] = findpeaks(i_icasigS{i}(j,:),fs_data,'MinPeakProminence',3);
        [pk_temp_neg,locs_temp_neg] = findpeaks(-1*i_icasigS{i}(j,:),fs_data,'MinPeakProminence',3);
        %
        ...
        [maxi,ind] = max(P1(f>90 & f<3000));
        f_temp= f(f>90 & f<3000);
        rounded_f = round(f_temp(ind),-1);
        
        % DJC 4-17-2017 - add in 200 Hz stim peak
        if ((~isempty(pk_temp_pos) || ~isempty(pk_temp_neg)) && total_peaks > 10 && mod(rounded_f,100) == 0)
