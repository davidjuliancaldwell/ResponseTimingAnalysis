function [evaluated_value,huberInside_mat,huberOutside_mat,processedSig,recon_artifact_matrix] =  ica_train_optimize_subtract_ResponseTiming(data,stimChans,fs_data,numComponentsSearch,scale_factor,meanSub,orderPoly,plotIt,channelInt,nonlinear)
...
for i = 1:numTrials
    ...
    for j = 1:numICs
       ...
       % findpeaks(-1*zscore(i_icasigS{i}(j,:)),fs_data,'MinPeakProminence',3);
        %         %
        ...
        diff_sig = [0 diff(i_icasigS{i}(j,:))]';
        
        % find channel that has the max signal
        [~,chanMax] = max(max(diff_sig));
        
        % find onset
        startInd = find(abs(zscore(diff_sig(:,chanMax)))>1.5,1,'first');
        endInd = find(abs(zscore(diff_sig(:,chanMax)))>1.5,1,'last');
        
        % check to make sure matches onset and offset of stimulation train
        if ~isempty(startInd) || ~isempty(endInd)
            trainCond = (abs((endInd-startInd)/fs_data) < 0.8);
        else
            trainCond = false;
        end
        
        art = zeros(length(i_icasigS{i}(j,:)),1);
        art(startInd:endInd) = 1;
        art = logical(art);
        % check to  make sure it doesnt have too much real signal
        ...
        if ((~isempty(pk_temp_pos) || ~isempty(pk_temp_neg)) && total_peaks > 10 && (mod(rounded_f,200) <= 5) &&  trainCond && noGoodSignal)
